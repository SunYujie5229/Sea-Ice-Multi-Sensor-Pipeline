...
def compute_cs2_density(cs2_gdf, class_col):
    ...
    if cs2_gdf.empty:
         return {
            "count_CS2_lead": 0, "count_CS2_ice": 0, "count_CS2_refrozen": 0, 
            "count_CS2_ambi": 0, "total_CS2": 0, # 新增 count_CS2_ambi
            "density_CS2_lead_only": 0.0, "density_CS2_leadref": 0.0, 
            "density_CS2_floe": 0.0, "density_CS2_floeref": 0.0,
            "density_CS2_ambiguous": 0.0  # 新增 density 键名
        }
    ...
    count_ambi     = (cs2_gdf[class_col] == "ambiguous").sum() # 新增统计
    
    # 注意：通常分母 total 应该包含所有有效点
    total = count_lead + count_ice + count_refrozen + count_ambi 
    ...
    return {
        ...
        "count_CS2_ambi": int(count_ambi), # 新增
        "total_CS2": int(total),
        ...
        "density_CS2_ambiguous": count_ambi / total # 新增
    }
...
def process_single_year(year):
    ...
    OUTPUT_DIR = rf"C:\Users\TJ002\Desktop\CS2_S1_result\overlap\20160114\{year}_density_simplest_filter"
 ...
 # 在 process_single_year 函数约第 250 行左右找到 combined 字典进行修改：
        combined = {
            ...
            "count_CS2_ambi": cs2_density["count_CS2_ambi"], # 新增
            ...
            "density_CS2_ambiguous": cs2_density["density_CS2_ambiguous"], # 新增
            
            # ... 后续 S1 的数据和 diff 保持不变 ...
