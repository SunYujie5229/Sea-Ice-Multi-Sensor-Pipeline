...
CONFIG = {
    ...
    # ── L2 数据根目录 ─────────────────────────────────────────
    # 目录结构约定：L2_BASE_DIR / YYYY / MM / <L2_nc>
    # 例：E:\Arctic_L2\2024\09\CS_OFFL_SIR_SIN_2__20240901T...nc
    # 若无 L2 数据，设为 None，对应字段填 NaN
    "L2_BASE_DIR": r"E:\Arctic_L2",   # 例：r"E:\Arctic_L2"
...
# L2 需要从文件读取的目标变量（保留原始变量名）
_L2_TARGET_VARS = [
    "sea_ice_concentration_01",   # 1Hz → broadcast 到 20Hz
    "snow_depth_cor_20_ku",       # 原生 20Hz
    "radar_freeboard_20_ku",      # 原生 20Hz
    "surf_type_20_ku",            # 原生 20Hz
]

# 哪些变量是 1Hz 需要 broadcast（通过 ind_first_meas_20hz_01 扩展）
_L2_BROADCAST_1HZ = {"sea_ice_concentration_01"}


def _parse_l1_stem(stem: str) -> dict | None:
    """
    从 L1 文件名 stem 解析出 mission/timeliness/product/start/end/version。

    L1 命名示例：
      CS_OFFL_SIR_SIN_1B_20220323T224728_20220323T224947_E001
    解析结果：
      mission=CS, timeliness=OFFL, instrument=SIR, product=SIN,
      level=1B, start=20220323T224728, end=20220323T224947, ver=E001
    """
    # 正则兼容 1B / 1b
    m = _re.match(
    ...
    return dict(
        mission     = m.group(1),
        timeliness  = m.group(2),
        instrument  = m.group(3),
        product     = m.group(4),
        level       = m.group(5),
        start       = m.group(6),   # e.g. 20220323T224728
        end         = m.group(7),
        version     = m.group(8),
    )
...
def _build_l2_stem(info: dict) -> str:
    """
    根据 L1 解析结果构造对应 L2 文件名 stem。

    L2 命名示例：
      CS_OFFL_SIR_SIN_2__20220323T224728_20220323T224947_E001
    差异：level 字段 '1B' → '2__'（两个下划线补足固定宽度）
    """
    return (
        f"{info['mission']}_{info['timeliness']}_{info['instrument']}_"
        f"{info['product']}_2__{info['start']}_{info['end']}_{info['version']}"
    )


def find_l2_file(l1_path: str, l2_base_dir: str | None) -> str | None:
    """
    根据 L1 文件路径，在 L2_BASE_DIR/YYYY/MM/ 下定位对应的 L2 nc 文件。

    搜索逻辑（优先级从高到低）：
      1. 精确文件名匹配（stem 完全一致）
      2. 同目录下文件名包含相同 start 时间戳的 nc 文件

    返回找到的 L2 文件完整路径，或 None。
    """
    if l2_base_dir is None:
        return None
    ...
    l2_dir = Path(l2_base_dir) / year_str / month_str
...
def broadcast_1hz_to_20hz(arr_1hz: np.ndarray,
    ...
    """
    将 1Hz 数组按 ind_first_meas_20hz_01 广播到 20Hz 长度。
    与 L1b 地球物理校正的处理方式完全一致。
    """
    out = np.full(n20, fill, dtype=float)
    for i, s in enumerate(ind_first):
        e = min(int(s) + 20, n20)
        out[int(s):e] = float(arr_1hz[i])
    return out
...
def read_l2_vars(l2_path: str,
    ...
    """
    读取 L2 nc 文件中的对比变量，统一输出为与 L1b lat_mask 对齐的数组。

    处理逻辑：
      - 1Hz 变量（sea_ice_concentration_01）：
          先用 ind_first_meas_20hz_01 broadcast 到 20Hz，再 apply lat_mask
      - 原生 20Hz 变量：直接 apply lat_mask
      - 变量不存在时：填 NaN，并记录警告

    返回 dict[变量名 → 1-D ndarray]，均与 L1b 过滤后的点数等长。
    """
    result = {v: np.full(lat_mask.sum(), np.nan) for v in _L2_TARGET_VARS}

    try:
        ds = xr.open_dataset(l2_path)
    except Exception as e:
        log.warning(f"  L2 文件打开失败: {l2_path} — {e}")
        return result

    n20_l2 = None   # L2 文件的 20Hz 长度（首次需要确定）

    # 优先读 ind_first（broadcast 1Hz 变量时需要）
    ind_first_l2 = None
    for idx_name in ("ind_first_meas_20hz_01", "ind_first_meas_20_ku_01"):
        if idx_name in ds.data_vars or idx_name in ds.coords:
            ind_first_l2 = ds[idx_name].values.astype(int)
            break

    for vname in _L2_TARGET_VARS:
        if vname not in ds.data_vars:
            log.debug(f"  L2 缺少变量: {vname}（填 NaN）")
            continue

        raw = ds[vname].values.astype(float)

        if vname in _L2_BROADCAST_1HZ:
            # ── 1Hz → 20Hz broadcast ────────────────────────────
            if ind_first_l2 is None:
                log.warning(f"  L2 缺少 ind_first_meas，无法 broadcast {vname}")
                continue

            # 推断 L2 的 20Hz 长度（用 lat_poca_20_ku 或 time_20_ku）
            if n20_l2 is None:
                for dim_var in ("lat_poca_20_ku", "lat_20_ku", "time_20_ku"):
                    if dim_var in ds.data_vars or dim_var in ds.coords:
                        n20_l2 = len(ds[dim_var].values)
                        break
                if n20_l2 is None:
                    log.warning(f"  L2 无法确定 20Hz 长度，跳过 broadcast {vname}")
                    continue

            arr_20 = broadcast_1hz_to_20hz(raw, ind_first_l2, n20_l2)

            # L2 的 20Hz 点数可能与 L1b 不同（同一 pass 但可能裁剪不同）
            # 采用长度对齐策略：取 min 长度后再 apply lat_mask
            n_common = min(len(arr_20), len(lat_mask))
            arr_aligned = np.full(len(lat_mask), np.nan)
            arr_aligned[:n_common] = arr_20[:n_common]
            result[vname] = arr_aligned[lat_mask]

        else:
            # ── 原生 20Hz 变量 ──────────────────────────────────
            n_common = min(len(raw), len(lat_mask))
            arr_aligned = np.full(len(lat_mask), np.nan)
            arr_aligned[:n_common] = raw[:n_common]
            result[vname] = arr_aligned[lat_mask]
...
def process_one_file(L1_path: str, cfg: dict) -> pd.DataFrame | None:
    ...
    # ── L2 对比变量 ───────────────────────────────────────────
    # 变量：sea_ice_concentration_01 / snow_depth_cor_20_ku /
    #        radar_freeboard_20_ku / surf_type_20_ku
    # 保留 L2 原始变量名，加前缀 "l2_" 避免与 L1 字段冲突
    l2_path = find_l2_file(L1_path, cfg.get("L2_BASE_DIR"))
    if l2_path:
        log.info(f"  找到对应 L2 文件: {os.path.basename(l2_path)}")
        l2_vars = read_l2_vars(l2_path, lat_mask)
        for vname, arr in l2_vars.items():
            df[f"l2_{vname}"] = arr
    else:
        log.debug("  未找到对应 L2 文件（L2 字段填 NaN）")
        for vname in _L2_TARGET_VARS:
            df[f"l2_{vname}"] = np.nan
    ...
    ORDERED_COLS = [
        ...
        *CORR_VARS,
        ...
        # 厚度
        "sit_raw",
        # ── L2 对比变量（原始变量名加 l2_ 前缀）──────────────
        "l2_sea_ice_concentration_01",   # SIC（L2 1Hz→broadcast）
        "l2_snow_depth_cor_20_ku",       # L2 雪深（20Hz，已校正）
        "l2_radar_freeboard_20_ku",      # L2 雷达干舷（20Hz）
        "l2_surf_type_20_ku",            # L2 地表类型（20Hz）
    ]
    # 保留所有列，缺失的列保持原样
