...
# -------------------- USER CONFIG --------------------
# 1. 定义年份变量
YEAR = 2018  # 您可以随时修改这里，例如改成 2022

# 2. 使用 f-string (rf"...") 将 YEAR 插入路径中
# 注意：rf 表示 raw f-string，既保留反斜杠转义功能，又支持变量替换
CS2_DIR = rf"C:\Users\TJ002\Desktop\CS2_S1_result\filter\{YEAR}\gpkg"
S1_DIR  = rf"C:\Users\TJ002\Desktop\CS2_S1_result\filter\{YEAR}\tif"
MATCH_CSV = rf"F:\NWP\CS2_S1_matched\time_match_{YEAR}_filter.csv"

# 输出目录也可以加上年份，方便区分
OUTPUT_DIR = rf"C:\Users\TJ002\Desktop\CS2_S1_result\overlap\{YEAR}_density_simplest_filter"
...
def plot_overlap_density_validation(s1_data, s1_transform, cs2_gdf_proj, scene_name):
    """
...
def compute_s1_density_point_sampling(s1_array, transform, cs2_points_gdf, win_radius=WINDOW_RADIUS):
    ...
    density_results = {
        "S1_lead_pixels": int(lead_total), "S1_ice_pixels": int(ice_total), "S1_refrozen_pixels": int(refr_total), "S1_total_pixels": int(total_pixels),
        "density_S1_lead_only": lead_total / total_pixels, "density_S1_leadref": (lead_total + refr_total) / total_pixels, "density_S1_floe": ice_total / total_pixels
    }
...
def main():
    print("=" * 80)
    print("CS2–S1 DENSITY CALCULATION SCRIPT (STRICT RASTER VALUE FILTER)")
    print("=" * 80)
    print(f"CS2 points are now filtered based on S1 classification value (NoData/Background = 0 excluded).")
    ...
    # --- Save summary ---
    if results:
        outcsv = os.path.join(OUTPUT_DIR, "overall_density_summary_{YEAR}_raster_filtered_detailed.csv")
