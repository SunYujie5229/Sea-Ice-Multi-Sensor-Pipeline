...
YEARS = [2015, 2016, 2017, 2019, 2020, 2021]  # 暂时跳过 2018, 2022
...
BASE_OUTPUT_DIR = r"F:\NWP\parameter_experiments"  # 实验输出根目录
...
def run_parameter_experiment():
    ...
    print("=" * 80)
    print(f"\nYears: {YEARS}")
    print(f"PP Floe Thresholds: {PP_FLOE_THRESHOLDS}")
    print(f"PP Lead Thresholds: {PP_LEAD_THRESHOLDS}")
    print(f"STD Thresholds: {STD_THRESHOLDS}")
    print(f"Total combinations: {len(PP_FLOE_THRESHOLDS) * len(PP_LEAD_THRESHOLDS) * len(STD_THRESHOLDS)}")
    
    # 创建总结果列表
    all_experiment_results = []
    
    # 遍历所有参数组合
    param_combinations = list(itertools.product(PP_FLOE_THRESHOLDS, PP_LEAD_THRESHOLDS, STD_THRESHOLDS))
    
    for exp_idx, (pp_floe, pp_lead, std_thresh) in enumerate(param_combinations, 1):
        ...
        # 创建实验输出目录
        exp_name = f"exp_PPfloe{pp_floe}_PPlead{pp_lead}_STD{std_thresh:.2f}"
        ...
        for year in YEARS:
            ...
            cs2_output_dir = os.path.join(exp_output_dir, "cs2_classified", str(year))
            match_csv = os.path.join(BASE_MATCH_CSV_DIR, f"time_match_{year}_filter.csv")
            s1_dir = os.path.join(BASE_S1_DIR, str(year), "tif")
            overlap_output_dir = os.path.join(exp_output_dir, "overlap_results", str(year))
        ...
        if yearly_rmsd_results:
            ...
            yearly_summary_path = os.path.join(exp_output_dir, "yearly_rmsd_summary.csv")
            ...
            all_experiment_results.append({
                ...
                "avg_rmsd_lead": avg_rmsd_lead,
                "avg_rmsd_ice": avg_rmsd_ice,
                "avg_mae_lead": df_yearly["mae_lead"].mean(),
                "avg_mae_ice": df_yearly["mae_ice"].mean(),
                "total_scenes": df_yearly["n_scenes"].sum(),
    ...
    if all_experiment_results:
        ...
        # 按 RMSD 排序
        df_all_sorted = df_all.sort_values("avg_rmsd_lead")
        
        summary_path = os.path.join(BASE_OUTPUT_DIR, "all_experiments_summary.csv")
        df_all_sorted.to_csv(summary_path, index=False)
        ...
        print("\nTop 5 parameter combinations (by Lead RMSD):")
        print(df_all_sorted[["experiment", "avg_rmsd_lead", "avg_rmsd_ice"]].head(10).to_string(index=False))
        
        # 生成最佳参数报告
        best_exp = df_all_sorted.iloc[0]
        print(f"\n🏆 BEST PARAMETERS:")
        print(f"   PP_floe < {best_exp['pp_floe_thresh']}")
        print(f"   PP_lead ≥ {best_exp['pp_lead_thresh']}")
        print(f"   STD ≤ {best_exp['std_thresh']:.2f}")
        print(f"   Avg RMSD (Lead): {best_exp['avg_rmsd_lead']:.4f}")
        print(f"   Avg RMSD (Ice):  {best_exp['avg_rmsd_ice']:.4f}")
