...
class SatelliteImageMatcher:
    ...
    def __init__(self, time_window_hours=2, cloud_threshold=20, min_overlap_ratio=0.3):
        """
        初始化匹配器
        ...
        Args:
            ...
            min_overlap_ratio: 最小重合率阈值（0-1之间）
        """
        # 初始化Earth Engine
        try:
            ee.Initialize()
        ...
        print(f"✓ 配置参数 - 时间窗口: {time_window_hours}小时, 云量阈值: {cloud_threshold}%, 最小重合率: {min_overlap_ratio*100}%")
    ...
    def calculate_overlap_ratio(self, geom1, geom2):
        ...
        try:
            # 计算相交区域
            intersection = geom1.intersection(geom2, maxError=1000)  # 1km误差容限
    ...
    def check_overlap_quality(self, s1_geometry, matched_geometry):
        """
        检查重合质量是否满足要求
        
        Args:
            s1_geometry: Sentinel-1几何
            matched_geometry: 匹配影像几何
            
        Returns:
            dict: 包含重合率和是否通过检查的信息
        """
        overlap_ratio = self.calculate_overlap_ratio(s1_geometry, matched_geometry)
        
        return {
            'overlap_ratio': round(overlap_ratio, 4),
            'passes_check': overlap_ratio >= self.min_overlap_ratio,
    ...
    def print_statistics(self):
        ...
        if 'overlap_percentage' in df.columns:
            print("\n=== 重合率统计 ===")
            overlap_data = df[df['overlap_percentage'].notna()]['overlap_percentage']
            if len(overlap_data) > 0:
                print(f"平均重合率: {overlap_data.mean():.2f}%")
                print(f"最小重合率: {overlap_data.min():.2f}%")
                print(f"最大重合率: {overlap_data.max():.2f}%")
                print(f"中位数重合率: {overlap_data.median():.2f}%")
                
                # 重合率分布
                ranges = [(0, 30), (30, 50), (50, 70), (70, 90), (90, 100)]
                print("\n重合率分布:")
                for min_val, max_val in ranges:
                    count = len(overlap_data[(overlap_data >= min_val) & (overlap_data < max_val)])
...
def main():
    ...
    # 方法2: 从本地CSV文件加载
    # scene_names = matcher.load_csv_file('path/to/your/csv_file.csv')
