...
#         S1_Floeref_desity = ('density_S1_floeref', 'mean'),
#         S1_Floeref_std    = ('density_S1_floeref', 'std'),
...
#         S1_Leadref_density = ('density_S1_leadref', 'mean'),
#         S1_Leadref_std     = ('density_S1_leadref', 'std'),
#     ).reset_index()
...
def compute_monthly_stats(df):
    summary = df.groupby("month").agg(
        ...
        # Ambiguous density (新增部分)
        CS2_Ambi_Density=('density_CS2_ambiguous', 'mean'),
        CS2_Ambi_Std    =('density_CS2_ambiguous', 'std'),
    ).reset_index()
...
# 2）处理全部年份（文件夹下 *.csv）
process_all_years(r"C:\Users\TJ002\Desktop\CS2_S1_result\overlap\20160114\statistic\*.csv")
