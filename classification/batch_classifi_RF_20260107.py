...
class Sentinel1IceClassifier:
    def __init__(self, selected_features=None):
        ...
        # TIFF波段名称 (与 Notebook In[7] 中的列表一致)
        self.tiff_band_names = [
                    "HH", "HV", "SIC", 
                    "HH_mult_HV", "HH_div_HV", "sum_div_diff", "HH_minus_HV"
                ]
        # 注意：Notebook 实际运行的 TIFF 可能有 ANGLE，但 CSV mapping 和 _classify_chunk 都没有使用 ANGLE 的计算逻辑。
        # 保持 Notebook 的 tiff_band_names 定义一致。
...
# 创建一个健壮的特征名称映射
        csv_feature_mapping = {
            ...
            'HV': 'HV',
            'HH_div_HV': 'HH_div_HV',
            'HH_minus_HV': 'HH_minus_HV', # 关键：添加此行
            'ANGLE': 'ANGLE',             # 关键：添加此行
            'sum_div_diff': 'sum_div_diff', # 关键：添加此行
            'HH_mult_HV': 'HH_mult_HV',     # 建议统一标准名

            # === Part 2: 别名映射 ===
            # 用于兼容旧的或不同格式的CSV文件
            'HH_dif_HV': 'HH_minus_HV',    # 'HH_dif_HV' 是 'HH_minus_HV' 的一个别名
            'Angle': 'ANGLE',     # 'indicenceAngle' 是 'ANGLE' 的一个别名
            'ratio': 'sum_div_diff',       # 'ratio' 是 'sum_div_diff' 的一个别名
            'HH_HV': 'HH_mult_HV'          # 'HH_HV' 是 'HH_mult_HV' 的一个别名
        }
    ...
    def check_tiff_bands(self, tiff_path):
        ...
        with rasterio.open(tiff_path) as src:
            ...
            print("波段对应关系:")
            # 使用self.tiff_band_names列表进行波段名称打印
            ...
            for feature in self.feature_names:
                ...
                elif feature in ['HH_div_HV', 'HH_minus_HV', 'sum_div_diff', 'HH_mult_HV']:
                    # 检查是否可以计算衍生特征
    ...
    def evaluate_model(self, X, y, output_folder):
        """评估模型性能 (使用 70/30 划分)"""
        print("\n评估模型性能...")
    ...
    def _classify_chunk(self, chunk_data):
        """对单个chunk进行分类 (已根据Notebook中的逻辑修复)"""
            ...
            if feature_name in bands_dict:
                ...
                if feature_name == 'HH_div_HV' and 'HH' in bands_dict and 'HV' in bands_dict:
                    calculated_band = np.divide(bands_dict['HH'], bands_dict['HV'], 
                                              out=np.zeros_like(bands_dict['HH'], dtype=np.float32), 
                    ...
                    feature_arrays.append(calculated_band)
                    available_features.append(feature_name)
                elif feature_name == 'HH_mult_HV' and 'HH' in bands_dict and 'HV' in bands_dict:
                    # 修复：添加对 HH_mult_HV 的计算，与 Notebook 中的逻辑一致
                    calculated_band = bands_dict['HH'] * bands_dict['HV']
                    feature_arrays.append(calculated_band)
                    available_features.append(feature_name)
        ...
        if 'SIC' in bands_dict:
            ...
            valid_mask = valid_mask & (sic_data > 10) & (sic_data <= 100)
            
        # 修复：确保至少有一个特征（例如HH）不为0，避免全0区域被分类。
        # Notebook 中实际使用的 HH != 0 的掩膜逻辑
        if 'HH' in bands_dict:
            # 创建一个仅针对 HH 波段的掩膜 (HH 波段不为0)
            hh_mask = (bands_dict['HH'] != 0)
            
            # 使用逻辑与(&)操作，将 HH 掩膜与之前的掩膜合并
            valid_mask = valid_mask & hh_mask
            
        # 确保其他特征带（如比值或差值）不包含 NaN/Inf
        # 检查所有实际使用的特征数组
        for arr in feature_arrays:
            valid_mask = valid_mask & ~np.isnan(arr) & ~np.isinf(arr)

        valid_pixels = valid_mask.flatten()
        
        # 处理无效值（将 NaN/Inf 替换为 0，因为它们在 valid_mask 中已经被排除了，这里是为了防止 RF 模型在预测时因 NaN/Inf 报错）
        ...
        if np.sum(valid_pixels) > 0:
            ...
            # 检查特征数量是否匹配 (调试用)
            if self.rf_model.n_features_in_ != valid_features.shape[1]:
                raise ValueError(f"分类特征数量不匹配: 模型期望 {self.rf_model.n_features_in_}，实际输入 {valid_features.shape[1]}")
    ...
    def visualize_classification(self, classification_result, profile, 
        ...
        # 确保 Matplotlib 可以在非交互式环境中使用
        if 'DISPLAY' not in os.environ:
             plt.switch_backend('Agg')

        fig, axes = plt.subplots(2, 2, figsize=(15, 12))
        
        if original_tiff_path and os.path.exists(original_tiff_path):
            ...
            # 如果原始文件不存在，用空白图替代或只显示分类结果
            axes[0, 0].set_title('HH波段 (文件未找到)')
            axes[0, 1].set_title('HV波段 (文件未找到)')
            axes[0, 0].axis('off')
            axes[0, 1].axis('off')
            ...
            for i, tiff_path in enumerate(tiff_files, 1):
                ...
                try:
                    ...
                    try:
                        ...
                        # plt.close('all') # 在 visualize_classification 中已关闭
...
def run_batch_classification_workflow():
    ...
    # 2. 选择特征组合或自定义特征 (与 Notebook In[27] 保持一致)
    ...
    # 4. 定义训练样本CSV文件 (保持路径与 Notebook 一致)
    csv_files = [
    ...
    # 假设批处理文件夹与单文件示例中的 TIFF 文件在同一父目录下，这里使用示例中找到的路径结构。
    input_folder = r"E:\pre_processing\2018"
    output_folder = r"C:\Users\TJ002\Desktop\classification result\2018" # 新建一个输出文件夹以区分
...
def run_single_file_classification():
    """
    单文件分类示例（与 Notebook In[27] 到 In[31] 流程一致）
    """
    print("=== 单文件分类示例 ===")
    
    # 使用自定义特征 (与 Notebook In[27] 保持一致)
    custom_features = ['HH', 'HV','HH_div_HV', 'HH_mult_HV']  
    classifier = Sentinel1IceClassifier(custom_features)
    
    # CSV训练文件 (保持路径与 Notebook 一致)
    csv_files = [
        r"H:\我的云端硬盘\Project_Yujie\RF Samples1\samplePoint_20230329T155110_20230329T155214_047861_05C03E_8378.csv",
        r"H:\我的云端硬盘\Project_Yujie\RF Samples1\samplePoint_20230412T153445_20230412T153549_048065_05C71E_CE44.csv",
        r"H:\我的云端硬盘\Project_Yujie\RF Samples1\samplePoint_20230426T151920_20230426T152020_048269_05CDF5_BEA4.csv",
    ...
    # 单文件分类的输入和输出路径
    # 使用 Notebook Cell In[16] 和 In[20] 的路径，但修复输出路径
    tiff_path = r"F:\NWP\S1_processed_for_classification\2023\S1A_EW_GRDM_1SDH_20230412T121730_20230412T121835_048063_05C70C_40A9_EW_HH_HV_angle_int16x100_87caa6ee_1_processed.tif"
    output_dir = r"F:\NWP\Classification Result\Single_File_Test"
    ...
    # 2. 评估
    classifier.evaluate_model(X, y, output_dir)
    classifier.get_cv_report(X, y, output_dir)
    
    # 3. 检查特征匹配和TIFF波段信息
    classifier.debug_feature_info()
    classifier.check_tiff_bands(tiff_path)
    
    # 4. 分块分类 (Notebook Cell In[20] 流程)
    print("开始单文件分块分类...")
